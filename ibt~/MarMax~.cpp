...
//  dsp_add(MarMax_perform64, 3, x, sp[0]->s_vec, sp[0]->s_n);
//}
...
//  t_MarMax *x = (t_MarMax *)(ins[1]);
  ...
  float output_tempo, curMedianTempo = 0;
  ...
  if (Nozero)
  {
    ...
    for (j = 0; j < vectorSize; j++) {
      ...
      if (x->pos == x->hopsize - 1) {
        ...
        output_tempo = (float) x->m_MarsyasNetwork->getControl("Series/featureNetwork/FlowThru/beattracker/BeatTimesSink/sink/mrs_natural/curMedianTempo")->to<mrs_natural>();
        if(output_tempo > 0) curMedianTempo = output_tempo;

//        post("tempo: %f", curMedianTempo);
